      ...
      {icons.map((icon) => (
        <li key={icon.id}>
          <div className="relative h-[6rem]">
            <button type="button" id={icon.id} aria-label={icon.name} className="absolute inset-0 flex h-full w-full cursor-auto items-center justify-center rounded-xl text-slate-900 ring-1 ring-inset ring-slate-900/[0.08]">
